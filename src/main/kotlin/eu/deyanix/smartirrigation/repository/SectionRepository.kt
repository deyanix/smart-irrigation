...
interface SectionRepository : JpaRepository<Section, Int> {
	@Query("SELECT S FROM Section S WHERE S.installation.id = :installationId AND S.index = :sectionIndex")
	fun findByIndex(installationId: Int, sectionIndex: Int): Optional<Section>

	@Query("SELECT S FROM Section S WHERE S.installation.id = :installationId ORDER BY S.index")
	fun findAllByInstallation(installationId: Int): Stream<Section>
}
